# Agents Store — Antigravity Plugins
...
No public marketplace exists for Antigravity yet. Clone this repository, then install a plugin directly:

```bash
agy plugin install ./agents-store-antigravity-skills/<plugin-name>
```

Copies the plugin to `.agents/plugins/<plugin-name>/` (workspace-scoped). For a global install at `~/.gemini/config/plugins/<plugin-name>/` instead:

```bash
agy plugin install --global ./agents-store-antigravity-skills/<plugin-name>
```
...
| [macstack-dev](./macstack-dev) | MACSTACK dev plugin for Agents Store. Creates and maintains the macstack/ folder of a Claude project: macstack.json — the standardized business + technical stac | 20 | 1 | 15 | — |
